...
export const ColorPickerFormat = ({
  ...
  if (mode === "hex") {
    return (
      <div
        className={cn(
          "flex w-full items-center rounded-md border border-input overflow-hidden",
          className
        )}
        {...(props as any)}
      >
        <Input
          className="h-8 flex-1 border-0 bg-secondary px-2 text-xs shadow-none focus-visible:ring-0"
          type="text"
          value={hexInput}
          onChange={handleHexChange}
          placeholder="#000000"
        />
        {showAlpha && <PercentageInput value={alpha} />}
      </div>
    );
  }
    ...
    return (
      ...
      >
        {rgb.map((value, index) => (
          <Input
            className={cn(
              "h-8 flex-1 border-0 bg-secondary px-2 text-xs text-center shadow-none focus-visible:ring-0",
  ...
  if (mode === "css") {
    ...
    return (
      ...
      >
        <Input
          className="h-8 w-full border-0 bg-secondary px-2 text-xs shadow-none focus-visible:ring-0"
          readOnly
          type="text"
          value={
            showAlpha
              ? `rgba(${rgb.join(", ")}, ${Math.round(alpha)}%)`
              : `rgb(${rgb.join(", ")})`
          }
        />
      </div>
